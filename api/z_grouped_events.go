...
func GroupedEvents(height int64) ([]*EventsResponseValidator, error) {
	...
	mapPubkeyEvents := map[string][]*compact_db.RewardEvent{}
	...
	for _, event := range events {
		...
		mapPubkeyEvents[e.ValidatorPubKey.String()] = append(mapPubkeyEvents[e.ValidatorPubKey.String()], e)
	}

	var responseValidators []*EventsResponseValidator
	for _, val := range tmVals.Validators {
		pubKey := types.BytesToPubkey(val.PubKey.Bytes()[5:]).String()
